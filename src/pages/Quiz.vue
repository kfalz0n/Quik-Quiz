<template>
   <div id="quiz-page">
       ...
       <v-form v-model="valid">
            <div class="question" v-for="(q, index) in questions" :key="index">
                ...
                <div class="question-answers">
                    <label v-for="answer in q.answers" :key="answer">
                        <div>
                            <input type="radio" v-model="userResponse[index]"
                            :name="q.id" :value="answer">
                            {{ answer }}
                        </div>
                    </label>
                </div>
            ...
            :counter="15" label="First Name" required></v-text-field>
            <v-text-field v-model="lastName" :rules="nameRules"
            :counter="15" label="Last Name" required></v-text-field>
        ...
        <v-card>
            ...
            <v-card-text>
                Hey {{quizUserResult.name | capitalize}}, you have scored
                {{quizUserResult.score}}% in your quiz! You got
                {{quizUserResult.correctAnswers}} of 5 correct
            </v-card-text>
...
export default {
  ...
  methods: {
    submitQuiz() {
      ...
      this.$store.dispatch('score/addScore', this.quizResult);
    },
    calculateScore() {
